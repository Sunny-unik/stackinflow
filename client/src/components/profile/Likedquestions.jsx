...
export default function Likedquestions() {
  ...
  return (
    <div className='border' style={{ minHeight: '46vh' }}>
      <h1 className='m-0' style={{ borderBottom: '2px solid lightgrey', paddingBottom: '2%' }}>
        ...
        return (
          <div className='m-0' style={{ border: '.1rem solid lightgrey' }}>
            <h4 className='mainqdiv'>
              <NavLink style={{ textDecoration: 'none' }} to={`/question/${g._id}`}>
                {g.question}
              </NavLink>
            </h4>
            ...
            <div className='maintagdiv mx-2'>
              {g.tags.map(o => {
                return (
                  <NavLink
                    style={{ color: 'white', fontFamily: 'monospace', padding: '.2rem' }}
            ...
            <div
              style={{ width: '37%', display: 'inline-block', margin: '6px', fontFamily: 'Times' }}
            >
              posted {handleDate(g.date) !== 0 ? handleDate(g.date) + ' day ago' : 'today'}
            </div>
          </div>
        );
      })}
