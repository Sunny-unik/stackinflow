...
<div className='border' style={{ minHeight:'46vh'}}>
    <h1 className='m-0' style={{ borderBottom  : "2px solid lightgrey", paddingBottom: "2%" }}>&nbsp;All following questions are liked by {user.dname}.</h1>
    {userliked.map((g) => {
        return <div className='m-0' style={{ border: '.1rem solid lightgrey' }}>
            <h4 className='mainqdiv'>
                <NavLink style={{ textDecoration: 'none' }} to={`/question/${g._id}`}>{g.question}</NavLink>
            </h4>
            ...
            <div class="maintagdiv mx-2">
                {g.tags.map((o)=>{
                return <NavLink style={{ color: 'white', fontFamily: 'monospace', padding: '.2rem' }} className="m-1 rounded-2 bg-dark" to={`/questionsby/${o}`}>
                        {o}
                    </NavLink>
                })}
            </div>
