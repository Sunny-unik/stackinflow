...
export default function Contact() {
    ...
    function submitfeedback() {
        if(!user){alert("For submit feedback you need to login first")}
        if(Feedbackhead.length>73 || Feedbackhead.length<4 || Feedbackhead==null || Feedbackhead==" ")
        {alert("Feedback title length must between 4 to 73")}
        if(Feedbackbody.length>1000 || Feedbackbody.length<10 || Feedbackbody==null || Feedbackbody==" ")
        {alert("Feedback body length must between 10 to 1000")}
        if(Feedbackfoot.length>500 || Feedbackfoot.length<6 || Feedbackfoot==null || Feedbackfoot==" ")
    ...
    return (
        ...
        <div className='card-header'>
            ...
            <input type="text" value={Feedbackhead} onChange={(e) => { handlemail(e) }} placeholder="Enter your feedback title" name="ehead"
            id="ehead" required style={{ width: "90%" }} /><br />
        </div>
            <br />
        <div className='card-body bg-light'>
            <label for="ebody"><b>Include all your reviews in brief</b></label><br />
            <textarea type="text" value={Feedbackbody} onChange={(e) => { handlemail(e) }} placeholder="Describe your views" name="ebody" id="ebody"
                required rows="7" style={{ width: "90%" }}></textarea><br />
        </div>
            <br />
        <div className='card-footer'>
            <label for="efoot"><b>Enter footer content or conclusion</b></label><br />
            <input type="text" value={Feedbackfoot} onChange={(e) => { handlemail(e) }} placeholder="Enter footer content or conclusion" name="efoot"
            id="efoot" required style={{ width: "90%" }} /><br /> <br />
        </div>
            <br />
