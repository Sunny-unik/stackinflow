...
var client = new Mongoclient(env.config().parsed.MONGO_URI, { useNewUrlParser: true, useUnifiedTopology: true });
var connection;
...
app.post("/send-otp-email", bodyparser.json(), (req, res) => {
    ...
    usercollection.find({ $or: [{ email: req.body.email }, { dname: req.body.email }] }).toArray((err, result) => {
        if (!err && result.length > 0) {
            ...
            sendMail("process.env.APP_ID", env.config().parsed.APP_PASSWORD, result[0].email, "Welcome to stackinflow", `Your One Time Password is - <h3>${req.body.otp}</h3><br><h6>We hope you find our service cool.</h6>`);
...
app.post("/update-question", bodyparser.json(), (req, res) => {
    var questioncollection = connection.db('stackinflow').collection('q&a');
    console.log(req.body);
    questioncollection.updateOne({ _id: ObjectId(req.body.questionid) }, {
        $set: {
            question: req.body.question,
            questiondetail: req.body.questiondetail, tags: req.body.tags
        }
    }, (err, result) => {
        if (!err) {
...
app.post("/update-user-details", bodyparser.json(), (req, res) => {
    var usercollection = connection.db('stackinflow').collection('user');
    console.log(req.body);
    usercollection.updateOne({ _id: ObjectId(req.body.obid) }, {
        $set: {
            name: req.body.name, dname: req.body.dname, title: req.body.title,
...
app.post("/send-user-otp", bodyparser.json(), (req, res) => {
    console.log(req.body);
    sendMail("process.env.APP_ID", env.config().parsed.APP_PASSWORD, req.body.email, "Welcome to stackinflow",
...
app.post('/send-feedback', bodyparser.json(), (req, res) => {
    ...
    feedbackcollection.insert(req.body, (err, result) => {
        if (!err) {
            ...
            sendMail("process.env.APP_ID", env.config().parsed.APP_PASSWORD, req.body.useremail, "Thanks for submit your feedback",
...
app.post('/create-user', bodyparser.json(), (req, res) => {
    ...
    usercollection.insert(req.body, (err, result) => {
        if (!err) {
            ...
            sendMail("process.env.APP_ID", env.config().parsed.APP_PASSWORD, req.body.email, "Welcome to stackinflow", `<h2>stackinflow</h2><br><h4> Registration SuccessFull </h4><br>We hope you find our service cool.`);
        }
        else {
            res.send({ status: "failed", data: err });
        }
...
app.listen(env.config().parsed.PORT, () => {
    console.log("Server is running on port " + env.config().parsed.PORT);
});